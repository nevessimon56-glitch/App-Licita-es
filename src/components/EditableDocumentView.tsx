...
import { isEditableSection } from "@/lib/editable-sections";
import {
...
function ReadOnlySection({ title, body }: { title: string; body: string }) {
  ...
  const blocks = parseDocumentMarkdown(sectionMarkdown).filter(
...
export function EditableDocumentView({ markdown, onMarkdownChange }: Props) {
  ...
  const firstTitleIndex = preambleBlocks.findIndex((b) => b.type === "title");

  const headerBlocks = preambleBlocks.filter((block, index) => {
    ...
    <article className="doc-report">
      ...
      <div className="doc-report-body">
        {headerBlocks.map((block, index) => (
          <DocBlock key={`preamble-${index}`} block={block} />
        ))}

        {sections.map((section) =>
          isEditableSection(section.title) ? (
            <EditableSectionEditor
              key={section.title}
              title={section.title}
              body={section.body}
